    ...
    def _generate_kernel_code(
        ...
        for op_dict in op_chain:
            ...
            if op.name == "block.matmul":
                # Need to move inputs to L0A and L0B
                input_a = inputs_list[0]
                input_b = inputs_list[1]

                # Generate move operations for matmul inputs
                input_a_l0a, move_lines_a = self._generate_matmul_memory_moves(input_a, 3, has_matmul)
                code_lines.extend(move_lines_a)

                input_b_l0b, move_lines_b = self._generate_matmul_memory_moves(input_b, 4, has_matmul)
