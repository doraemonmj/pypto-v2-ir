...
class OpFuzzer:
    ...
    BLOCK_REDUCTION_OPS = [
        OpSpec(
            ...
            lambda a, tmp: np.sum(a, axis=1, keepdims=True),
        ...
        OpSpec(
            ...
            "tile",
            {"produces_row_vec": True, "requires_tmp_tile": True},
            lambda a, tmp: np.max(a, axis=1, keepdims=True),
            shape_transform=lambda shapes: (shapes[0][0], 1) if len(shapes) >= 1 else (128, 1),
        ),
        OpSpec(
            "block.row_min",
            ...
            "tile",
            {"produces_row_vec": True, "requires_tmp_tile": True},
            lambda a, tmp: np.min(a, axis=1, keepdims=True),
            shape_transform=lambda shapes: (shapes[0][0], 1) if len(shapes) >= 1 else (128, 1),
        ),
    ]
    ...
    def __init__(
        ...
        if enable_advanced_ops:
            # Add reduction and row_expand operators
            self.advanced_vector_ops = self.BLOCK_REDUCTION_OPS + self.BLOCK_ROW_EXPAND_OPS
    ...
    def _get_eligible_ops_safe(  # noqa: PLR0912
        ...
        5. Select operator pool based on probability (only for V-pipe)
        """
        ...
        if self.current_pipe_type == "M":
            ...
            # For V-pipe with advanced ops, select based on probability
            if self.rng.random() < self.advanced_ops_probability:
                candidate_ops = self.advanced_vector_ops
            else:
                candidate_ops = self.basic_vector_ops
        else:
            candidate_ops = self.ops
    ...
    def generate_numpy_reference(  # noqa: PLR0912
        self,
