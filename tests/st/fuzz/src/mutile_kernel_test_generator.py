...
class MultiKernelTestGenerator:
    ...
    def _get_torch_operation(self, op_name: str, input_vals: List[str]) -> str:  # noqa: PLR0912
        """PyPTO  Torch

        Args:
            op_name: PyPTO  ( "block.add")
            input_vals:
        ...
        # Row expand
            ...
            return f"{input_vals[0]} / {input_vals[1]}"
        # Row reduction  ( [M, 1] )
        elif op_name == "block.row_sum":
            # tile，tile（）
            return f"torch.sum({input_vals[0]}, dim=1, keepdim=True)"
        elif op_name == "block.row_max":
            return f"torch.max({input_vals[0]}, dim=1, keepdim=True)[0]"
        elif op_name == "block.row_min":
            return f"torch.min({input_vals[0]}, dim=1, keepdim=True)[0]"
